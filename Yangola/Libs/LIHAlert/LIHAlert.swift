...
@objc open class LIHAlert: NSObject {
    ...
    open var  touchBackgroundHandler: (() -> Void)?
    open var  dimsBackground: Bool = false
    open var backgroundDimOpacity: CGFloat = 0.4 {
        didSet {
            self.dimView?.alpha = self.backgroundDimOpacity
        }
    }
...
extension LIHAlert: UIGestureRecognizerDelegate {
    
    public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch) -> Bool {
        
        if let ol = self.overlayView {
            
            let point = touch.location(in: ol)
